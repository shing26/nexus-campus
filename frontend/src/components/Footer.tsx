...
function AgentTicker() {
  ...
  const { data: logsData } = useQuery({
    ...
    queryFn: async () => (await apiClient.get('/agent-logs', { params: { page: 1, size: 6 } })).data,
    staleTime: 1000 * 60,
    refetchInterval: 1000 * 60,
  });

  const logs: AiLog[] = logsData?.data?.list ?? [];
  ...
  const status = active?.isApproved === 1 ? 'APPROVED' : active ? 'FLAGGED' : 'IDLE';
    ...
    <div className="border-b border-vibe-border bg-vibe-bg/70">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 py-2 min-w-0">
          ...
          <span className={'shrink-0 text-[9px] font-mono ring-1 ring-inset rounded px-1.5 py-0.5 ' + (active?.isApproved === 1 ? 'text-vibe-emerald ring-vibe-emerald/30' : active ? 'text-amber-400 ring-amber-500/40' : 'text-slate-600 ring-vibe-border')}>
            {status}
          </span>
